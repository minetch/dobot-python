...
dobot studioのscript機能を使ってコードを実行できることを確認しました(dobot_scriptディレクトリ)

dobot studioの環境依存になるためscriptを実行するにあたって各dobotのpythonバージョンを公式のドキュメントから確認してください（私は面倒だったので確認しませんでした）

今回確認できたのはmatch文が実行できないところまでバージョンの確認ができました
